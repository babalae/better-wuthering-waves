...
using BetterWutheringWaves.GameTask.Model.Area.Converter;
    ...
    public Region Find(RecognitionObject ro, Action<Region>? successAction = null, Action? failAction = null)
    {
        ...
        if (RecognitionTypes.TemplateMatch.Equals(ro.RecognitionType))
        {
            ...
            if (ro.RegionOfInterest != Rect.Empty)
            {
                ...
                {
                    TaskControl.Logger.LogError("在图像{W1}x{H1}中查找模板,名称：{Name},ROI位置{X2},{Y2},区域{H2}x{W2},边界溢出！", roi.Width, roi.Height, ro.Name, ro.RegionOfInterest.X, ro.RegionOfInterest.Y, ro.RegionOfInterest.Width, ro.RegionOfInterest.Height);
                }
